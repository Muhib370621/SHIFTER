...
class OrderController extends GetxController {
  ...
  driverGetOrderByID(String orderID) async {
    try {
      isLoading.value = true;
      var result = await AppServices().driverGetByOrderID(
          // phoneNumberController.value.text,
          // otpCode.value,
          orderID);

      orderModel.value = result.value?.data??OrderModel();
      // listOfNewOrders.value = result.value?.data?.dataList??[];
      // Prompts.successSnackBar("User Verified Successfully!");
      // final User userModel = User(
      //   id: result.value?.data?.id.toString(),
  ...
  changeOrderStatus(String orderID, int orderStatus) async {
    try {
      isLoading.value = true;
      var result = await AppServices().changeOrderStatus(
          // phoneNumberController.value.text,
          // otpCode.value,
          orderID,orderStatus);
